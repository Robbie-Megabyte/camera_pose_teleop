...
class LatestFrameCamera:
    ...
    def __init__(
        ...
        cmd = [
            ...
            # Output 1: unchanged GVHMR BGR24 feed.
            "-map",
            "0:v:0",
...
_KP_PREVIEW_CONTEXT = None
_KP_PREVIEW_SOCKET = None


def publish_keypoint_preview(frame):
    """
    Best-effort/latest-only JPEG preview.

    tcp://127.0.0.1:5601
      - same camera frame
      - red ViTPose keypoints
      - green skeleton
      - no visible person bounding box

    This publisher must never block the inference loop.
    """
    global _KP_PREVIEW_CONTEXT
    global _KP_PREVIEW_SOCKET

    if _KP_PREVIEW_SOCKET is None:
        _KP_PREVIEW_CONTEXT = zmq.Context.instance()

        _KP_PREVIEW_SOCKET = _KP_PREVIEW_CONTEXT.socket(
            zmq.PUB
        )

        _KP_PREVIEW_SOCKET.setsockopt(
            zmq.SNDHWM,
            1,
        )
        ...
        _KP_PREVIEW_SOCKET.bind(
            "tcp://127.0.0.1:5601"
        )

        print(
            "ViTPose overlay preview: "
            "tcp://127.0.0.1:5601"
        )
