...
def main():
    ...
    # Latest-only keypoint preview publisher.
    #
    # This is localhost-only and completely independent
    # from SONIC's port 5556 publisher.
    kp_preview_context = zmq.Context.instance()

    kp_preview_socket = kp_preview_context.socket(
        zmq.PUB
    )

    kp_preview_socket.setsockopt(
        zmq.SNDHWM,
        1,
    )
    ...
    kp_preview_socket.bind(
        "tcp://127.0.0.1:5601"
    )

    print(
        "ViTPose preview publisher: "
        "tcp://127.0.0.1:5601"
    )
    ...
    def process_frame(
        ...
        # -------------------------------------------------
        # LIVE ViTPose-17 preview
        #
        # Draw the exact keypoints already used by GVHMR,
        # then publish only the newest JPEG to localhost.
        # No GUI calls are made in the inference process.
        # -------------------------------------------------
        ...
        for a, b in skeleton_edges:
            if (
                kp_np[a, 2] > confidence_threshold
                and
                kp_np[b, 2] > confidence_threshold
            ):
                pa = (
                    int(round(kp_np[a, 0])),
                    int(round(kp_np[a, 1])),
        ...
        for joint in range(17):
            if (
                ...
                point = (
                    int(round(kp_np[joint, 0])),
                    int(round(kp_np[joint, 1])),
                )
        ...
        if ok:
            try:
                kp_preview_socket.send(
                    encoded_preview.tobytes(),
                    flags=zmq.NOBLOCK,
                )

            except zmq.Again:
                # Preview is explicitly best-effort.
                # Never stall inference for visualization.
                pass
